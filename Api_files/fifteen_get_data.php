...
function fetch_stocks() {
    ...
    $getcategories = "SELECT * FROM fifteen_min_details WHERE article_id = ".$stock_id." ORDER BY Date DESC";
    $resultscategory = $conn->query($getcategories);
    $data = array();
    if ($resultscategory->num_rows > 0) {            
        while($row = $resultscategory->fetch_assoc()) {
...
function add_daily_details() {
    ...
    // $getcategories = "INSERT INTO fifteen_min_details (article_name, article_id,Open, High, Low, Close, Date) VALUES ('".$stock_text."', '".$stock_val."','".$open_val."','".$high_val."','".$low_val."','".$close_val."','".$date_val."')";


    // if (mysqli_query($conn, $getcategories)) {
        $check_query = "SELECT * FROM fifteen_min_details WHERE article_name = '$stock_text' AND article_id = '$stock_val' AND Date = '$date_val'";

        $check_result = mysqli_query($conn, $check_query);
        if (mysqli_num_rows($check_result) == 0) {
            // Insert only if no duplicate is found
            $query = "INSERT INTO fifteen_min_details (article_name, article_id, Date, Open, High, Low, Close) 
            VALUES (
                '$stock_text',
                '$stock_val',
                '$date_val',
            ...
            if (!mysqli_query($conn, $query)) {
                ...
                $query = "INSERT INTO fifteen_min_calculation (article_name, article_id, low,support,high,calc_first,calc_two,calc_trend,date,daily_id) VALUES ";
                ...
                foreach ($rows as $rowData) {
                    ...
                    $valuesArr[] = "('$article_name', '$article_id', '$level_one_in_low', '$level_one_in_support', '$level_one_in_high', '$level_one_in_supp_high', '$level_one_in_supp_low','$level_one_in_up_down','$date','$daily_id')";
                }
    ...
    if ($zip->open($fileTmpPath) === TRUE) {
        ...
        if ($xmlString !== false) {
            ...
            foreach ($xml->sheetData->row as $row) {
                ...
                $excelStartDate = '1900-01-01 00:00:00';
                
                if (isset($rowData[0]) && is_numeric($rowData[0])) {
                    // Convert float to integer explicitly to handle timestamp precision
                    $timestamp = (int) (strtotime($excelStartDate) + ($rowData[0] - 2) * 86400);
                
                    // Convert to datetime format
                    $final_date_val = date("Y-m-d H:i:s", $timestamp);
                } else {
                    // Fallback for invalid input, default to the current Excel start date
                    $final_date_val = date("Y-m-d H:i:s", strtotime($excelStartDate));
                }
                ...
                $check_query = "SELECT * FROM fifteen_min_details WHERE article_name = '$stock_text' AND article_id = '$stock_val' AND Date = '$final_date_val'";
                $check_result = mysqli_query($conn, $check_query);
...
function get_data_stocks_dash() {
    $conn = db_connect();
    $getcategories = "SELECT*FROM fifteen_min_details LIMIT 30";
    $resultscategory = $conn->query($getcategories);
    $data = array();
    if ($resultscategory->num_rows > 0) {            
        while($row = $resultscategory->fetch_assoc()) {
...
function delete_details_trunc() {
    ...
    //     $query1 = "DELETE FROM fifteen_min_details WHERE article_id = ?";
    ...
    //         throw new Exception("No record found in `fifteen_min_details` with id $stock_id.");
    //     }

    //     // Delete from the second table
    //     $query2 = "DELETE FROM fifteen_min_calculation WHERE article_id = ?";
    ...
    //         throw new Exception("No record found in `fifteen_min_calculation` with id $stock_id.");
    //     }
    ...
    // }
    $response = ["success" => false, "message" => ""];
    $getcategories = "DELETE FROM fifteen_min_details WHERE article_id = '$stock_id'";

    if (mysqli_query($conn, $getcategories)) {
        $response["success"] = true;
        $response["message"] = "Records deleted successfully from both tables.";
        $getcategories2 = "DELETE FROM fifteen_min_calculation WHERE article_id = '$stock_id'";
        if (mysqli_query($conn, $getcategories2)) {
            $response["success"] = true;
            $response["message"] = "Records deleted successfully from both tables.";
        }else{
            $response = ["success" => false, "message" => "No record found in `fifteen_min_calculation` with id $stock_id."];
...
function dele_new_details_details() {
    ...
    $getcategories = "DELETE FROM fifteen_min_details WHERE id = '$stock_id'";

    if (mysqli_query($conn, $getcategories)) {
        $getcategories1 = "DELETE FROM fifteen_min_calculation WHERE daily_id = '$stock_id'";
...
function get_daily_counts() {
    $conn = db_connect();
    $getcategories = "SELECT COUNT(*) AS total_count FROM fifteen_min_details";
    $resultscategory = $conn->query($getcategories);
    $data = array();
    if ($resultscategory->num_rows > 0) {            
        while($row = $resultscategory->fetch_assoc()) {
...
function get_weekly_counts() {
    $conn = db_connect();
    $getcategories = "SELECT COUNT(*) AS total_count FROM weekly_details";
    $resultscategory = $conn->query($getcategories);
    $data = array();
    if ($resultscategory->num_rows > 0) {            
        while($row = $resultscategory->fetch_assoc()) {
...
function get_monthly_counts() {
    $conn = db_connect();
    $getcategories = "SELECT COUNT(*) AS total_count FROM monthly_details";
    $resultscategory = $conn->query($getcategories);
    $data = array();
    if ($resultscategory->num_rows > 0) {            
        while($row = $resultscategory->fetch_assoc()) {
