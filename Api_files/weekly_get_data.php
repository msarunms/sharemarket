...
function fetch_stocks() {
    ...
    $getcategories = "SELECT * FROM weekly_details WHERE article_id = ".$stock_id." ORDER BY Date DESC";
    $resultscategory = $conn->query($getcategories);
    $data = array();
    if ($resultscategory->num_rows > 0) {            
        while($row = $resultscategory->fetch_assoc()) {
...
function add_daily_details() {
    ...
    // $getcategories = "INSERT INTO weekly_details (article_name, article_id,Open, High, Low, Close, Date) VALUES ('".$stock_text."', '".$stock_val."','".$open_val."','".$high_val."','".$low_val."','".$close_val."','".$date_val."')";


    // if (mysqli_query($conn, $getcategories)) {
        $check_query = "SELECT * FROM weekly_details WHERE article_name = '$stock_text' AND article_id = '$stock_val' AND Date = '$date_val'";

        $check_result = mysqli_query($conn, $check_query);
        if (mysqli_num_rows($check_result) == 0) {
            // Insert only if no duplicate is found
            $query = "INSERT INTO weekly_details (article_name, article_id, Date, Open, High, Low, Close) 
            VALUES (
                '$stock_text',
                '$stock_val',
                '$date_val',
            ...
            if (!mysqli_query($conn, $query)) {
                ...
                $query = "INSERT INTO weekly_calculation (article_name, article_id, low,support,high,calc_first,calc_two,calc_trend,date,weekly_id) VALUES ";
                ...
                foreach ($rows as $rowData) {
                    ...
                    $valuesArr[] = "('$article_name', '$article_id', '$level_one_in_low', '$level_one_in_support', '$level_one_in_high', '$level_one_in_supp_high', '$level_one_in_supp_low','$level_one_in_up_down','$date','$weekly_id')";
                }
    ...
    if ($zip->open($fileTmpPath) === TRUE) {
        ...
        if ($xmlString !== false) {
            ...
            foreach ($xml->sheetData->row as $row) {
                ...
                $check_query = "SELECT * FROM weekly_details WHERE article_name = '$stock_text' AND article_id = '$stock_val' AND Date = '$final_date_val'";
                $check_result = mysqli_query($conn, $check_query);
...
function get_data_stocks_dash() {
    $conn = db_connect();
    $getcategories = "SELECT*FROM weekly_details LIMIT 30";
    $resultscategory = $conn->query($getcategories);
    $data = array();
    if ($resultscategory->num_rows > 0) {            
        while($row = $resultscategory->fetch_assoc()) {
...
function delete_details_trunc() {
    ...
    // }
    $response = ["success" => false, "message" => ""];
    $getcategories = "DELETE FROM weekly_details WHERE article_id = '$stock_id'";

    if (mysqli_query($conn, $getcategories)) {
        $response["success"] = true;
        $response["message"] = "Records deleted successfully from both tables.";
        $getcategories2 = "DELETE FROM weekly_calculation WHERE article_id = '$stock_id'";
...
function dele_new_details_details() {
    ...
    $getcategories = "DELETE FROM weekly_details WHERE id = '$stock_id'";

    if (mysqli_query($conn, $getcategories)) {
        $getcategories1 = "DELETE FROM weekly_calculation WHERE weekly_id = '$stock_id'";
